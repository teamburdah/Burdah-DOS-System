...
STATIC VOID CfgOemId(BYTE * pLine);
...
STATIC struct table commands[] = {
  ...
  {"OEMID", 1, CfgOemId},         /* Burdah: OEMID=xx (hex), e.g. OEMID=0xFD */
...
STATIC BOOL SkipLine(char *pLine)
{
  ...
  if (originalskipconfigseconds >= 0)
  {
    ...
    else if (key == 0x011b || (key & 0x00ff) == 0x1b) /* ESC */
    {
      splash_close();
    }
...
STATIC VOID CfgOemId(BYTE * pLine)
{
  ...
  /* Get the argument; accepts decimal or 0x-prefixed hex, e.g.
     OEMID=0xBB or OEMID=187. Lets a "stubborn" program be served
     a different OEM ID (e.g. OEMID=0xFD to masquerade as FreeDOS)
     without recompiling the kernel. */
  if (GetNumArg(pLine, &val) == (BYTE *) 0)
    return;

  if (InitKernelConfig.Verbose >= 0) printf("Changing reported OEM ID to 0x%02X\n", val & 0xff);
