...
md source
cd source
git clone https://github.com/teamburdah/Burdah-DOS-System/portal.git
cd portal
...
del .gitignore
cd ..
7z a -mx9 -tzip ..\portal-source.zip .
popd
...
copy docs\html\commands\Portal.html %pkgdir%\DOC\COMMAND\Portal.HTM
copy utils\MAKECMD.BAT %pkgdir%\NLS\MAKECMD.BAT
:: language specific versions copied into NLS during build
copy build\portal-source.zip %pkgdir%\SOURCE\PORTAL\SOURCES.ZIP
cd %pkgdir%
